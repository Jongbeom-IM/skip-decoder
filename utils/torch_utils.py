...
def model_spliter(model_path, decoder_path, split_point, device):
    from models.yolo import Decoder, Model
    
    current_dir = os.getcwd()
    cfg_path = os.path.join(current_dir, 'cfg', 'deploy', f"e6e_SD1.yaml")

    if isinstance(model_path, list):
        pretrained = [torch.load(w, map_location=device) for w in model_path]
    else:
        pretrained = torch.load(model_path, map_location=device)
    # decoder = custom_load(decoder_path).to(device).half()
    ...
    prefix = ([f'model.{i}.' for i in range(int(split_point)+1)], 
              [f'model.{i}.' for i in range(int(split_point)+1, int(prefix)+1)])
