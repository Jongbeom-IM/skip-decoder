...
class HDF5FolderDataset(Dataset):
    def __init__(self, h5_folder_path):
        self.path = h5_folder_path
        self.h5_files = [os.path.join(self.path, file) \
                         for file in os.listdir(self.path) if file.endswith('.h5')]
        self.dataset_info_x = self._get_dataset_info()

    def _get_dataset_info(self):
        datasets = []
        for x in self.h5_files:
            with h5py.File(x, 'r') as f:
                for dataset_name in f.keys():
                    datasets.append((x, dataset_name)) # key: 9
        return datasets

    def __len__(self):
        return len(self.dataset_info_x)

    def __getitem__(self, idx):
        h5_file, dataset_name = self.dataset_info_x[idx]
        with h5py.File(h5_file, 'r') as f:
            data = np.array(f[str(dataset_name)], dtype=np.float32)
